<h1 align="left ">Hi there, I'm Sazzad Hossain</h1>
<h3 align="left">Frontend Developer | Aspiring WordPress Plugin & Theme Developer</h3>

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=sazzadhossainmit&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

## About Me

I have been building powerful and dynamic WordPress websites with modern tools
and custom functionalities for the past 4 years. During this journey, I actively
sought deeper knowledge by learning from senior developers and leveraging
extensive online resources.

This experience helped me understand the importance of creating secure, fast,
clean, and real-world web solutions. I enjoy solving client problems by adding
custom PHP/JS functionalities instead of relying on unnecessary heavy plugins.

Currently, I am expanding my horizons by diving deep into **Advanced &
Object-Oriented PHP (OOP)** and modern **JavaScript** to build next-generation
custom WordPress themes and plugins, while leveraging cutting-edge **AI tools**
to optimize workflows and stay ahead of the tech curve.

Beyond that, I am also looking forward to expanding my knowledge of Laravel and
other PHP and JavaScript frameworks.

## Currently Learning

```
- Advanced PHP
- OOP Concepts
- JavaScript Logic
- Custom Theme Development
- Custom WordPress Theme & Plugin Development
- Modern WordPress Architecture
```

---

## Tech Stack & Skills

### Frontend

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,php" />
</p>

### Backend & CMS

<p>
  <img src="https://skillicons.dev/icons?i=php,wordpress" />
</p>

### Builders & Tools that I use mostly

- Elementor
- Divi Builder
- WPBakery
- Bricks Builder
- JetEngine
- ACF (Advanced Custom Fields)
- WooCommerce
- Crocoblock
- Custom PHP/JS Features

---

## GitHub Stats

<p align="center">
  <img
    src="https://github-readme-stats-sigma-five.vercel.app/api?username=sazzadhossainmit&show_icons=true&theme=tokyonight"
    alt="GitHub Stats"
  />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=sazzadhossainmit&theme=tokyonight" alt="streak stats" />
</p>

---

## Connect With Me

<p>
  <a href="https://github.com/sazzadhossainmit" target="_blank">
    <img src="https://skillicons.dev/icons?i=github" />
  </a>

  <a href="https://linkedin.com/in/sazzadhossainmit" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>

<a href="mailto:sazzadhossainmit@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
</p>
